...
init : () -> ( Model, Cmd Msg)
init _  =
    (Model "" "" "" How False False, Cmd.none)
        ...
        case model.route of
            How ->
                div []
                    ...
                    , p [ ] [ text "2017sukeijinkou.csv、新潟市、クリエイティブ・コモンズ・ライセンス 表示 2.1 日本" ]
                ...
                case route of
                    ...
                    Top ->
                        div [] [  p [] [ text "平均値について" ]
                                ...
                                , p [] [ text "標準偏差について" ]
                                , p [] [ text "標準偏差とはデータの平均からどれくらいのブレがあるのかを示す指標のこと、この数値がでかいと良くも悪くも平均からずれていることになる。" ]
                        ]
                    ...
                    Ols ->
                        div [] 
                            ...
                            , p [] [ text "その中から最も2乗和したときの値が小さくなる数値を求める方法である。" ]
                            , p [] [ text "二乗和した値が最も小さいということは誤差が最も小さいということで、これが正解に最も近い関係式が求められる。" ]
                            , p [] [ text "例えば特定の人の数学の点数が良いと英語の点数が良くなる傾向があるのかどうかといった問題に使える。" ]
                            ]
                    ...
                    Dist ->
                        div [] 
                            ...
                            , p [] [ text "繰り返し実行していきながらグラフを作っていくとある一定の分布を形成していく、これを二項分布という、あらゆる自然現象において発生する。" ]
                            , p [] [ text "データの平均が取れているときに測定した結果特定の結果になる確率を分布してみたものをポアソン分布という。" ]
                            ]

                    Parcen ->
                        div [] 
                            [ p [] [ text "確率とは" ]
                            , p [] [ text "一つの事象が起きる可能性のことである。ありえる事象の一つのことを標本点といい、それらが寄り集まって集合したものを標本空間と呼ぶ" ]
                            , p [] [ text "何も起こらない事象にも空事象という名前がある。" ]
                            , p [] [ text "階乗という特定の数値から一個ずつ引いた数をかけていく計算方法を組み合わせの数を求めるときに使う方法がある。" ]
                            , p [] [ text "階乗計算の考え方を応用したものとして順列と組み合わせの数が求められる。"]
                            , p [] [ text "順列とは順序をつけて1列に並べた配列のことで全体の個数をnとして取り出す数をrと定義すると" ]
                            , p [] [ text "nPrと表現される。" ]
                            , p [] [ text "ものの順序を考えないで並べた場合は組み合わせの数と言われ" ]
                            , p [] [ text "nCrと表現される。" ]
                            ]
                    ...
                    Hypo ->
                        div []
                            [ p [] [ text "仮説検定とは" ]
                            , p [] [ text "名前通りに提唱した仮説が正しいかどうか検証すること" ]
                            , p [] [ text "統計における仮説がただしいかどうかを決めるやり方は以下のとおりである。" ]
                            , p [] [ text "まず自分が作ったもともとの仮説を" ]
                            , strong [] [ text "帰無仮説" ]
                            , p [] [ text "と定義付け" ]
                            , p [] [ text "それに対立する仮説を" ]
                            , strong [] [ text "対立仮説" ]
                            , p [] [ text "として定義し、" ]
                            , p [] [ text "対立仮説の是非でもともとの仮説をありかなしか検証する。" ]
                            , p [] [ text "仮に対立仮説が否定されたとしてももともとの仮説こと帰無仮説を積極的に肯定したわけではないことがポイント" ]
                            , p [] [ text "帰無仮説のほうが正しいかもとなっただけである。" ]
                            ]
            ]
        ]
